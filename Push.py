        ...
        if (time.localtime()[3] <= 5) and nightvideo: #time.localtime()[3] >= 23 or 
            ...
            if(night_files[night_ran].find('.mp3') != -1):  #如果为mp3
                ...
                if not os.path.isfile(path+'/resource/night/'+night_files[night_ran]+'.ass'):
                    service.AssMaker.make_ass('../night/'+night_files[night_ran].replace('.mp3',''),'当前是晚间专属时间哦~时间范围：凌晨0-5点\\N大家晚安哦~做个好梦~\\N当前文件名：'+night_files[night_ran],path)
                print('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/night/'+night_files[night_ran]+'" -vf ass="'+path+'/resource/night/'+night_files[night_ran]+'.ass" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                print('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/night/'+night_files[night_ran]+'" -vf ass="'+path+'/resource/night/'+night_files[night_ran]+'.ass" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                os.system('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/night/'+night_files[night_ran]+'" -vf ass="'+path+'/resource/night/'+night_files[night_ran].replace('.mp3','')+'.ass" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
            continue
        ...
        for f in files:
            if((f.find('.mp3') != -1) and (f.find('.download') == -1)): #如果是mp3文件
                ...
                if((seconds > 600) | (bitrate > 400000)):  #大于十分钟就不播放/码率限制400k以下
                    ...
                    pic_ran = random.randint(0,len(pic_files)-1)    #随机选一张图片
                    #推流
                    print('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/playlist/'+f+'" -vf ass="'+path+"/resource/playlist/"+f.replace(".mp3",'')+'.ass'+'" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                    os.system('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/playlist/'+f+'" -vf ass="'+path+"/resource/playlist/"+f.replace(".mp3",'')+'.ass'+'" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                ...
                count+=1    #点播统计加一
                break
            # if((f.find('ok.flv') != -1) and (f.find('.download') == -1) and (f.find('rendering') == -1)):   #如果是有ok标记的mp4文件
                # print('flv:'+f)
                # #直接推流
        ...
        if(count == 0):     #点播统计为0，说明点播的都放完了
            ...
            if(mp3_files[mp3_ran].find('.mp3') != -1):  #如果是mp3文件
                ...
                if(os.path.isfile(path+'/resource/music/'+mp3_files[mp3_ran].replace(".mp3",'')+'.ass')):
                    print('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+"/resource/music/"+mp3_files[mp3_ran].replace(".mp3",'')+'.ass'+'" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                    os.system('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+"/resource/music/"+mp3_files[mp3_ran].replace(".mp3",'')+'.ass'+'" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                else:
                    print('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+'/resource/music/default.ass" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                    os.system('ffmpeg -threads 1 -re -loop 1 -r 2 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+'/resource/music/default.ass" -x264-params "profile=high:level=4.0" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
            # if(mp3_files[mp3_ran].find('.flv') != -1):  #如果为flv视频
                # #直接推流
                # print('ffmpeg -threads 1 -re -i "'+path+"/resource/music/"+mp3_files[mp3_ran]+'" -vcodec copy -acodec copy -f flv "'+rtmp+live_code+'"')
                # os.system('ffmpeg -threads 1 -re -i "'+path+"/resource/music/"+mp3_files[mp3_ran]+'" -vcodec copy -acodec copy -f flv "'+rtmp+live_code+'"')
    except Exception as e:
        print(e)
