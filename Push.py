        ...
        if(count == 0):     #点播统计为0，说明点播的都放完了
            ...
            if(mp3_files[mp3_ran].find('.mp3') != -1):  #如果是mp3文件
                ...
                if(os.path.isfile(path+'/resource/music/'+mp3_files[mp3_ran].replace(".mp3",'')+'.ass')):
                    print('ffmpeg -threads 1 -re -loop 1 -r 15 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+"/resource/music/"+mp3_files[mp3_ran].replace(".mp3",'')+'.ass'+'" -x264-params "profile=high:level=5.1" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                    os.system('ffmpeg -threads 1 -re -loop 1 -r 15 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+"/resource/music/"+mp3_files[mp3_ran].replace(".mp3",'')+'.ass'+'" -x264-params "profile=high:level=5.1" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                else:
                    print('ffmpeg -threads 1 -re -loop 1 -r 15 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+'/resource/music/default.ass" -x264-params "profile=high:level=5.1" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
                    os.system('ffmpeg -threads 1 -re -loop 1 -r 15 -t '+str(int(seconds))+' -f image2 -i "'+path+'/resource/img/'+pic_files[pic_ran]+'" -i "'+path+'/resource/music/'+mp3_files[mp3_ran]+'" -vf ass="'+path+'/resource/music/default.ass" -x264-params "profile=high:level=5.1" -pix_fmt yuv420p -b '+config['rtmp']['bitrate']+'k -vcodec libx264 -acodec copy -f flv "'+rtmp+live_code+'"')
